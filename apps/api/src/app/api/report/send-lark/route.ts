...
import { runDailyCrawl, runDailyLarkReport } from "@daily-news/core";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const report = await runDailyLarkReport();
    ...
    return withCors(
      req,
      NextResponse.json({ ok: false, error: "Failed to send lark report" }, { status: 500 })
    );
  }
}
